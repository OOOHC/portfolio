...
var BLOGS_FILE = import_path.default.join(DATA_DIR, "blogs.json");
async function getBlogs() {
  try {
    await import_promises.default.mkdir(DATA_DIR, { recursive: true });
    try {
      const data = await import_promises.default.readFile(BLOGS_FILE, "utf-8");
      return JSON.parse(data);
    } catch {
      await import_promises.default.writeFile(BLOGS_FILE, JSON.stringify(DEFAULT_POSTS, null, 2), "utf-8");
...
async function saveBlogs(blogs) {
  try {
    await import_promises.default.mkdir(DATA_DIR, { recursive: true });
    await import_promises.default.writeFile(BLOGS_FILE, JSON.stringify(blogs, null, 2), "utf-8");
    return true;
  } catch (error) {
...
async function getProjects() {
  try {
    await import_promises.default.mkdir(DATA_DIR, { recursive: true });
    try {
      const data = await import_promises.default.readFile(PROJECTS_FILE, "utf-8");
      return JSON.parse(data);
    } catch {
      await import_promises.default.writeFile(PROJECTS_FILE, JSON.stringify(DEFAULT_PROJECTS, null, 2), "utf-8");
...
async function saveProjects(projects) {
  try {
    await import_promises.default.mkdir(DATA_DIR, { recursive: true });
    await import_promises.default.writeFile(PROJECTS_FILE, JSON.stringify(projects, null, 2), "utf-8");
    return true;
  } catch (error) {
...
async function startServer() {
  ...
  app.put("/api/blogs/:id", requireAuth, async (req, res) => {
    ...
    blogs[idx] = {
      ...
      draft: draft !== void 0 ? !!draft : blogs[idx].draft
    };
    const success = await saveBlogs(blogs);
    if (success) {
      res.json(blogs[idx]);
  ...
  app.put("/api/projects/:id", requireAuth, async (req, res) => {
    ...
    projects[idx] = {
      ...
      draft: draft !== void 0 ? !!draft : projects[idx].draft
    };
    const success = await saveProjects(projects);
    if (success) {
